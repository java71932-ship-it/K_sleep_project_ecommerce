...
public class Controllers {
    ...
    public void initAdmin() {
        List<Entitysignup> admins = sr.findByEmail("admin@ksleep.com");
        if (admins.isEmpty()) {
            Entitysignup admin = new Entitysignup();
            admin.setName("pankaj");
            admin.setEmail("admin@ksleep.com");
            admin.setPassword(passwordEncoder.encode("Pankaj@3287"));
            admin.setRole("ADMIN");
            sr.save(admin);
        } else {
            Entitysignup admin = admins.get(0);
    ...
    public String adminLogin(
        ...
        List<Entitysignup> admins = sr.findByEmail("admin@ksleep.com");
        if (!admins.isEmpty()) {
            passwordMatches = passwordEncoder.matches(password, admins.get(0).getPassword());
        } else {
    ...
    public List<prodectentity> fechdata() {
        ...
        return pr.findAll().stream().map(product -> {
            ...
            if (product.getImage1() != null && !product.getImage1().startsWith("/images/")
                    && !product.getImage1().startsWith("http"))
                product.setImage1("/images/" + product.getImage1());
            if (product.getImage2() != null && !product.getImage2().startsWith("/images/")
                    && !product.getImage2().startsWith("http"))
                product.setImage2("/images/" + product.getImage2());
            if (product.getImage3() != null && !product.getImage3().startsWith("/images/")
                    && !product.getImage3().startsWith("http"))
                product.setImage3("/images/" + product.getImage3());
            if (product.getImage4() != null && !product.getImage4().startsWith("/images/")
                    && !product.getImage4().startsWith("http"))
                product.setImage4("/images/" + product.getImage4());
            if (product.getImage5() != null && !product.getImage5().startsWith("/images/")
    ...
    public ResponseEntity<Resource> getImage(@PathVariable String imageName) throws IOException {
        ...
        // Fallback: Search dynamically if not found in default uploadDir
        if (!Files.exists(imagePath) || !Files.isReadable(imagePath)) {
            Path[] fallbackPaths = {
                    Paths.get("..", "images").resolve(imageName),
                    Paths.get("images").resolve(imageName),
                    Paths.get("..", "project.image").resolve(imageName),
                    Paths.get("project.image").resolve(imageName),
                    Paths.get("C:/Users/pankaj kumar saini/Documents/Ecommerce/images/").resolve(imageName),
                    Paths.get("C:/Users/pankaj kumar saini/Desktop/Ecommerce_zip_deploye/Ecommerce/images/")
                            .resolve(imageName),
                    Paths.get("C:/Users/pankaj kumar saini/Desktop/Ecommerce_zip_deploye/Ecommerce/project.image/")
                            .resolve(imageName)
            };
        ...
        // If still not found locally, redirect to Supabase Storage
        if (!Files.exists(imagePath) || !Files.isReadable(imagePath)) {
            String supabaseStorageUrl = "https://jcdjgkfbfwxsxnzlblet.supabase.co/storage/v1/object/public/images/"
                    + imageName;
            return ResponseEntity.status(HttpStatus.FOUND)
                    .location(URI.create(supabaseStorageUrl))
                    .build();
        }
    ...
    public String paymentgateway(
        ...
        try {
            ...
            String loginEmail = (authentication != null && authentication.isAuthenticated()) ? authentication.getName()
                    : null;

            if (loginEmail == null || "anonymousUser".equals(loginEmail)) {
    ...
    public String sinuppage(
        ...
        es.setName(username);
        es.setEmail(email);
        es.setPassword(passwordEncoder.encode(password));
    ...
    public String loginpage(
        ...
        try {
            ...
            Entitysignup member = null;
            if (!users.isEmpty()) {
                member = users.get(0);
                if (passwordEncoder.matches(password, member.getPassword())) {
                    loginSuccess = true;
                } else if (password.equals(member.getPassword())) {
                    // Fallback to plain text matching for existing records & auto upgrade it to
                    // BCrypt
    ...
    public String cancelOrder(@RequestParam Long id,
        ...
        String mobileno = order.getMobile_No() != null ? order.getMobile_No() : "N/A";
    ...
    public String adminCustomers(Model model) {
        ...
        // Map email to userEntity profile (for reliable, case-insensitive matching in
        // Thymeleaf)
